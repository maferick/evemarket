...
<section class="surface-primary">
    ...
    <?php else: ?>
        ...
        <div class="mt-3 table-shell"><table class="table-ui"><thead><tr class="border-b border-border/70 text-xs uppercase text-muted"><th class="px-3 py-2 text-left">Side</th><th class="px-3 py-2 text-right">Ship type</th><th class="px-3 py-2 text-right">Observed (s)</th><th class="px-3 py-2 text-right">Baseline (s)</th><th class="px-3 py-2 text-right">Lift</th><th class="px-3 py-2 text-right">Samples</th></tr></thead><tbody><?php foreach ($hullAnomalies as $anomaly): ?><tr class="border-b border-border/40"><td class="px-3 py-2"><?= htmlspecialchars((string) ($anomaly['side_key'] ?? ''), ENT_QUOTES) ?></td><td class="px-3 py-2 text-right"><?= (int) ($anomaly['victim_ship_type_id'] ?? 0) ?></td><td class="px-3 py-2 text-right"><?= htmlspecialchars(number_format((float) ($anomaly['hull_survival_seconds'] ?? 0), 2), ENT_QUOTES) ?></td><td class="px-3 py-2 text-right"><?= htmlspecialchars(number_format((float) ($anomaly['baseline_survival_seconds'] ?? 0), 2), ENT_QUOTES) ?></td><td class="px-3 py-2 text-right"><?= htmlspecialchars(number_format((float) ($anomaly['survival_lift'] ?? 0), 3), ENT_QUOTES) ?></td><td class="px-3 py-2 text-right"><?= (int) ($anomaly['sample_count'] ?? 0) ?></td></tr><?php endforeach; ?></tbody></table></div>
