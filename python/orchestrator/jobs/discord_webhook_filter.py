...
def _collect_deal_alerts(db: SupplyCoreDb) -> list[dict[str, Any]]:
    ...
    rows = db.fetch_all(
        ...
        "       da.observed_at, "
        "       COALESCE(t.entity_name, CONCAT('Type #', da.item_type_id)) AS item_name "
        "FROM market_deal_alerts_current da "
        "LEFT JOIN entity_metadata_cache t ON t.entity_type = 'type' AND t.entity_id = da.item_type_id "
