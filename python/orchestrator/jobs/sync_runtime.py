...
SYNC_JOB_CONTRACTS: dict[str, dict[str, Any]] = {
    ...
    "killmail_r2z2_sync": {"job_key": "killmail_r2z2_sync", "title": "Killmail R2Z2 Sync", "description": "Ingest killmails from R2Z2 stream, filter by tracked entities, persist events.", "category": "sync", "execution_mode": "python", "supports_dry_run": True, "supports_batching": True, "supports_resume": True, "default_batch_size": 50, "checkpoint_strategy": "sequence_id", "lock_policy": {"allow_overlap": False, "scope": "killmail_ingestion"}, "source_systems": ["r2z2", "esi"], "output_tables": ["killmail_events", "killmail_event_payloads", "killmail_attackers", "killmail_items"]},
}
