<aside class="sidebar-shell">
    ...
    <nav class="space-y-5">
        <?php foreach (nav_groups() as $group): ?>
            <div>
                <div class="mb-2 px-1">
                    <p class="text-[0.7rem] font-semibold uppercase tracking-[0.22em] text-slate-500"><?= htmlspecialchars($group['heading'], ENT_QUOTES) ?></p>
                </div>
                <div class="space-y-1">
                    <?php foreach ($group['items'] as $item): ?>
                        <?php
                            ...
                            if ($item['path'] === '/') {
                                ...
                                $isParentActive = $requestPath === $item['path'] || str_starts_with($requestPath, rtrim($item['path'], '/') . '/');
                            }
                        ...
                        <div class="nav-group">
                            ...
                            <?php if ($item['children'] !== []): ?>
                                <div class="mt-1 space-y-0.5 border-t border-white/6 pt-1">
